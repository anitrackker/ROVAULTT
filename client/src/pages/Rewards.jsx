  ...
  const redeemPromoCode = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/promo/redeem`, {
        ...
        body: JSON.stringify({ username: user.name, code: promoCodeInput })
      });
      const data = await res.json();
      if (data.success) {
        updateBalance(data.vault);
        setPromoCodeInput('');
    ...
    <div className="rewards-page">
      ...
      <div className="rewards-grid">
        ...
        <div className="reward-card">
          ...
          <input type="text" placeholder="Enter code..." value={promoCodeInput} onChange={e => setPromoCodeInput(e.target.value)} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', padding: '10px', borderRadius: '8px', color: '#fff', marginBottom: '15px' }} />
          <button className="rc-btn" onClick={redeemPromoCode}>Redeem Code</button>
        </div>
      </div>
